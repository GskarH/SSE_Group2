...
def mfa_is_enabled(username):
    ...
    username = ? and mfa_enabled = 1", (username, )).fetchone()
...
def mfa_disable(username):
    ...
    c.execute("UPDATE users SET mfa_enabled = 0 WHERE \
    username = ?", (username,))
    conn.commit()
...
def mfa_enable(username):
    ...
    c.execute("UPDATE users SET mfa_enabled = 1 WHERE \
    username = ?", (username,))
    conn.commit()
...
def mfa_reset_secret(username):
    ...
    c.execute("UPDATE users SET mfa_secret = ? WHERE \
